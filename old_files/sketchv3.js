 ...
 *        Game constants
 * ============================= */
// Define the Google Apps Script URL once
const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbz4_GPMFJ8c4AWEgwb4yUOmWoxZVf59dOEiwA5NB2oV2760CpMkJhQ38g1_SdyslcfT/exec"; // replace with your actual URL
...
function showLoginScreen(deviceId) {
  ...
  // Fetch existing username from Google Sheets via Apps Script API
  fetch(`${GOOGLE_SCRIPT_URL}?action=profile&deviceId=${deviceId}`)
    .then(response => response.json())
    .then(data => {
      if (data.ok && data.profile) {
        // Fill in the existing username
        document.getElementById('usernameInput').value = data.profile.username;
      } else {
        // Suggest a username if no profile exists
        document.getElementById('usernameInput').value = `Player-${deviceId.substring(0, 6)}`;
      }
    });
  ...
  document.getElementById('submitUsername').onclick = function() {
    ...
    if (username) {
      // Validate username availability via API
      fetch(`${GOOGLE_SCRIPT_URL}?action=checkUsername&username=${username}`)
        .then(response => response.json())
        .then(data => {
          if (data.ok && data.available) {
            // Store the username in the profile sheet
            fetch(GOOGLE_SCRIPT_URL, {
              method: 'POST',
              body: JSON.stringify({ action: 'setUsername', deviceId, username })
            }).then(() => {
              // Proceed with the game after saving the username
              startGame();
            });
          } else {
...
// Assuming deviceId is available or generated
const deviceId = 'SOME_UNIQUE_DEVICE_ID';
showLoginScreen(deviceId);
